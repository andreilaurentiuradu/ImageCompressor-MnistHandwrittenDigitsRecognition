...
function new_X = task3 (photo, pcs)
  ...
  % asa mai departe.
  % luam dimensiunea matricii patratice ce contine
  % valorile proprii pe diagonala principala
  [x, x] = size(S);
  col = zeros(x, 1);

  % sortam descrescator valorile proprii in matrice si interschimbam coloanele
  % matricii cu vectorii proprii pe baza lor
  for i = 1 : x - 1
    for j = i + 1 : x
      if S(i, i) < S(j, j)
        temp = S(i, i);
